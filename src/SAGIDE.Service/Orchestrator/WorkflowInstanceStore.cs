...
/// </summary>
internal sealed class WorkflowInstanceStore
{
    // ── Shared runtime state ───────────────────────────────────────────────────

    /// <summary>instanceId → (WorkflowInstance, WorkflowDefinition)</summary>
    internal readonly ConcurrentDictionary<string, (WorkflowInstance Inst, WorkflowDefinition Def)> Active = new();

    /// <summary>taskId → (instanceId, stepId) — reverse lookup for OnTaskUpdateAsync</summary>
    internal readonly ConcurrentDictionary<string, (string InstanceId, string StepId)> TaskToStep = new();

    /// <summary>Per-instance semaphore — serialises DAG evaluation on a single instance</summary>
    internal readonly ConcurrentDictionary<string, SemaphoreSlim> Locks = new();

    /// <summary>instanceId → reverse adjacency map (dependencyId → list of dependent stepIds)</summary>
    internal readonly ConcurrentDictionary<string, Dictionary<string, List<string>>> RevDepsCache = new();
    ...
    internal WorkflowInstanceStore(
    ...
    internal void Add(WorkflowInstance inst, WorkflowDefinition def)
    {
    ...
    internal SemaphoreSlim GetLock(string instanceId) => Locks[instanceId];

    internal int Count => Active.Count;

    // ── Common helpers ─────────────────────────────────────────────────────────

    internal void BroadcastUpdate(WorkflowInstance inst) =>
        _eventBus.Publish(new WorkflowUpdatedEvent(inst));

    internal void PublishApprovalNeeded(string instanceId, string stepId, string prompt) =>
        _eventBus.Publish(new WorkflowApprovalNeededEvent(instanceId, stepId, prompt));

    internal async Task PersistAsync(WorkflowInstance inst)
    {
    ...
    /// </summary>
    internal void ScheduleCleanup(string instanceId)
    {
        _ = Task.Run(async () =>
        {
    ...
    internal static Dictionary<string, List<string>> BuildReverseDeps(WorkflowDefinition def)
    {
